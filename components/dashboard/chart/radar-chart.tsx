'use client';

import { useMemo } from 'react';
import {
  PolarAngleAxis,
  PolarGrid,
  Radar,
  RadarChart as RechartsRadarChart,
} from 'recharts';

import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from '@/components/ui/card';
import {
  ChartConfig,
  ChartContainer,
  ChartLegend,
  ChartLegendContent,
  ChartTooltip,
  ChartTooltipContent,
} from '@/components/ui/chart';

import { type ChartProps } from '../shell';

const chartConfig = {
  first: {
    label: 'Baseline',
    color: 'var(--chart-1)',
  },
  recentAvg: {
    label: 'Recent trend (avg. last 3)',
    color: 'var(--chart-3)',
  },
  latest: {
    label: 'Latest',
    color: 'var(--chart-2)',
  },
} satisfies ChartConfig;

interface RadarDataPoint {
  metric: string;
  first: number;
  recentAvg: number;
  latest: number;
}

export function RadarChartView({ data }: ChartProps) {
  const radarData = useMemo<RadarDataPoint[]>(() => {
    if (data.length === 0) return [];

    const sorted = [...data].sort(
      (a, b) =>
        new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
    );

    const first = sorted[0];
    const latest = sorted[sorted.length - 1];

    // Compute recent average (last 3 data points)
    const recentSlice = sorted.slice(-3);
    const recentAvg = {
      avg_efficiency:
        recentSlice.reduce((s, r) => s + r.avg_efficiency, 0) /
        recentSlice.length,
      avg_f_patient:
        recentSlice.reduce((s, r) => s + r.avg_f_patient, 0) /
        recentSlice.length,
      area:
        recentSlice.reduce((s, r) => s + r.area, 0) / recentSlice.length,
      avg_sparc:
        recentSlice.reduce((s, r) => s + r.avg_sparc, 0) /
        recentSlice.length,
    };

    // Compute min/max across all data for normalization
    const stats = {
      avg_efficiency: { min: Infinity, max: -Infinity },
      avg_f_patient: { min: Infinity, max: -Infinity },
      area: { min: Infinity, max: -Infinity },
      avg_sparc: { min: Infinity, max: -Infinity },
    } as Record<string, { min: number; max: number }>;

    for (const row of data) {
      for (const key of Object.keys(stats)) {
        const v = row[key as keyof typeof row] as number;
        if (v < stats[key].min) stats[key].min = v;
        if (v > stats[key].max) stats[key].max = v;
      }
    }

    const normalize = (value: number, key: string) => {
      const { min, max } = stats[key];
      if (max === min) return 50;
      return ((value - min) / (max - min)) * 100;
    };

    return [
      {
        metric: 'Path Efficiency',
        first: normalize(first.avg_efficiency, 'avg_efficiency'),
        recentAvg: normalize(recentAvg.avg_efficiency, 'avg_efficiency'),
        latest: normalize(latest.avg_efficiency, 'avg_efficiency'),
      },
      {
        metric: 'Force',
        first: normalize(first.avg_f_patient, 'avg_f_patient'),
        recentAvg: normalize(recentAvg.avg_f_patient, 'avg_f_patient'),
        latest: normalize(latest.avg_f_patient, 'avg_f_patient'),
      },
      {
        metric: 'Range of Motion',
        first: normalize(first.area, 'area'),
        recentAvg: normalize(recentAvg.area, 'area'),
        latest: normalize(latest.area, 'area'),
      },
      {
        metric: 'SPARC',
        first: normalize(first.avg_sparc, 'avg_sparc'),
        recentAvg: normalize(recentAvg.avg_sparc, 'avg_sparc'),
        latest: normalize(latest.avg_sparc, 'avg_sparc'),
      },
    ];
  }, [data]);

  const sessionCount = data.length;

  return (
    <Card className="w-full pt-0">
      <CardHeader className="gap-1 border-b px-6 py-6">
        <CardTitle>Overall Progress</CardTitle>
        <CardDescription>
          At-a-glance comparison across all metrics ({sessionCount} session
          {sessionCount !== 1 ? 's' : ''}, normalized 0–100)
        </CardDescription>
      </CardHeader>
      <CardContent className="flex items-center justify-center">
        {radarData.length > 0 ? (
          <ChartContainer
            config={chartConfig}
            className="aspect-square max-h-[400px] min-h-[300px] w-full"
          >
            <RechartsRadarChart data={radarData}>
              <ChartTooltip
                content={<ChartTooltipContent className="w-[200px]" />}
              />
              <PolarAngleAxis dataKey="metric" />
              <PolarGrid />
              <Radar
                name="Baseline"
                dataKey="first"
                fill="var(--color-first)"
                fillOpacity={0.15}
                stroke="var(--color-first)"
                strokeWidth={2}
              />
              <Radar
                name="Recent trend (avg. last 3)"
                dataKey="recentAvg"
                fill="var(--color-recentAvg)"
                fillOpacity={0.15}
                stroke="var(--color-recentAvg)"
                strokeWidth={2}
                strokeDasharray="4 4"
              />
              <Radar
                name="Latest"
                dataKey="latest"
                fill="var(--color-latest)"
                fillOpacity={0.2}
                stroke="var(--color-latest)"
                strokeWidth={2}
              />
              <ChartLegend content={<ChartLegendContent />} />
            </RechartsRadarChart>
          </ChartContainer>
        ) : (
          <p className="text-muted-foreground text-sm">No data available</p>
        )}
      </CardContent>
    </Card>
  );
}
